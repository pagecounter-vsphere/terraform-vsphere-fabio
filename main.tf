...
resource "vsphere_virtual_machine" "lb-vm" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "curl -sLo /tmp/fabio.sh https://raw.githubusercontent.com/kikitux/curl-bash/master/provision/fabio.sh",
      "sudo -E bash /tmp/fabio.sh",
    ]
  }
}

output "guest_ip_address" {
  value = vsphere_virtual_machine.lb-vm.guest_ip_addresses[0]
}
